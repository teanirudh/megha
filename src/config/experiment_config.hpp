...
inline ExperimentConfig parse_experiment_config(std::istream& in) {
    ...
    auto get_u32 = [&](const std::string& key, std::uint32_t def) -> std::uint32_t {
        auto it = kv.find(key);
        if (it == kv.end()) return def;
        return static_cast<std::uint32_t>(std::stoul(it->second));
    };
    auto get_u64 = [&](const std::string& key, std::uint64_t def) -> std::uint64_t {
        auto it = kv.find(key);
        if (it == kv.end()) return def;
    ...
    auto parse_list_str = [&](const std::string& key) -> std::vector<std::string> {
    ...
    auto parse_list_size_t = [&](const std::string& key) -> std::vector<std::size_t> {
    ...
    auto parse_list_u32 = [&](const std::string& key) -> std::vector<std::uint32_t> {
    ...
    auto parse_list_u64 = [&](const std::string& key) -> std::vector<std::uint64_t> {
    ...
    cfg.functions_per_tenant = get_u32("functions_per_tenant", cfg.functions_per_tenant);
    cfg.prewarm_enabled      = get_bool("prewarm.enabled", false);
    cfg.prewarm_per_function = get_u32("prewarm.per_function", cfg.prewarm_per_function);
    ...
    cfg.hetero_small_cpu_scale= get_f64("hetero.small_cpu_scale", cfg.hetero_small_cpu_scale);
    cfg.hetero_small_mem_scale= get_f64("hetero.small_mem_scale", cfg.hetero_small_mem_scale);
    ...
    if (cfg.attacker_enabled) {
        cfg.attacker_cfg.attacker_tenant   = static_cast<TenantId>(
            get_u32("attacker.tenant", 999));
        cfg.attacker_cfg.attacker_function = static_cast<FunctionId>(
            get_u32("attacker.function", 100));
        cfg.attacker_cfg.attack_per_victim = get_f64("attacker.attack_per_victim", 1.0);
    ...
    cfg.service_time.model = get_str("service_time.model", "exponential") == "fixed"
