  ...
  public:
    static void run(const ExperimentConfig &cfg)
    {
        ...
        bool has_sweep =
            ...
            !cfg.sweep_num_workers.empty() || !cfg.sweep_max_queue_lens.empty();
        if (!has_sweep)
        {
            // record elapsed time
        ...
        std::vector<double> attack_intensities =
            cfg.sweep_attack_intensities.empty()
                ? std::vector<double>{cfg.attacker_enabled
        ...
        std::vector<std::string> attacker_patterns =
            cfg.sweep_attacker_patterns.empty()
                ? std::vector<std::string>{cfg.attacker_enabled
                            ...
                            for (auto nw : num_workers)
                            {
                                for (auto mql : max_queue_lens)
                                {
                                    ...
                                    if (c.attacker_enabled)
                                    {
                                        ...
                                        c.attacker_cfg.total_invocations =
    ...
    static void run_single(const ExperimentConfig &cfg)
    {
        ...
        if (cfg.hetero_enabled)
        {
            ...
            if (frac > 1.0)
                frac = 1.0;
            num_small =
                static_cast<std::uint32_t>(std::round(frac * num_workers));
        }
        ...
        for (std::uint32_t i = 0; i < num_workers; ++i)
        {
            ...
            std::cout << "  worker " << i << ": cpu=" << wv.capacity.cpu_cores
                      << " mem=" << wv.capacity.memory_mb
                      << "MB storage=" << wv.capacity.storage_mb << "MB\n";
        }

        // 1.5 Pre-warm containers if enabled
        if (cfg.prewarm_enabled && cfg.prewarm_per_function > 0 &&
            cfg.num_workers > 0)
        {
            std::cout << "Pre-warming " << cfg.prewarm_per_function
                      << " containers per function across " << cfg.num_workers
                      << " workers\n";
            ...
            for (FunctionId fid : function_ids)
            {
                ...
                {
                    WorkerId wid =
                        static_cast<WorkerId>((func_idx + j) % num_workers);
        ...
        {
            throw std::runtime_error("Unsupported workload type: " +
                                     cfg.workload_type);
        }
        ...
        for (auto &kv : m.tenant_invocations())
        {
            std::cout << "  tenant " << kv.first << " -> "
                      << m.drops_for_tenant(kv.first) << "\n";
        }
        ...
        if (!cfg.output_csv.empty())
        {
            ...
            if (write_header)
            {
                out << "scheduler,workload,attacker,"
                    ...
                    << "service_time_model,service_time_mean,service_time_"
                       "fixed,"
                    << "worker_queue_len\n";
            }
            ...
            out << cfg.scheduler_name << "," << cfg.workload_type << ","
                << (cfg.attacker_enabled ? 1 : 0) << "," << cfg.num_tenants
                << "," << cfg.num_workers << "," << total_arrivals_all << ","
                << cfg.seed << "," << cfg.idle_timeout << ","
                << cfg.prewarm_per_function << ","
                << cfg.attacker_cfg.attack_per_victim << "," // attack_intensity
                << cfg.attacker_cfg.pattern << ","           // attacker_pattern
                << eng.now() << "," << eng.num_failed_invocations() << ","
                ...
                << "," << victim_drop_rate << "," << victim_mean_lat << ","
                << victim_p95_lat << ","
                << (cfg.service_time.model == ServiceTimeModel::FIXED ? "fixed"
                                                                      : "exp")
