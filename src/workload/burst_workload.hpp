...
public:
    BurstWorkload(std::uint32_t num_tenants,
                  ...
                  FunctionId    base_func_id        = 1,
                  TenantId      base_tenant_id      = 1,
                  std::uint64_t seed                = 0,
        ...
        , burst_duty_cycle_(burst_duty_cycle)
        , base_func_id_(base_func_id)
    ...
    enum class ServiceTimeModel { FIXED, EXPONENTIAL };
    ...
    std::vector<Invocation> next_batch(TimePoint now) override {
        ...
        if (to_gen == 0) return batch;

        batch.reserve(to_gen);

        std::uniform_int_distribution<std::uint32_t> tenant_dist(0, num_tenants_ - 1);
        ...
        for (std::uint32_t i = 0; i < to_gen; ++i) {
            ...
            std::uniform_int_distribution<std::uint32_t> func_dist(0, functions_per_tenant_ - 1);
            auto f_idx = func_dist(rng_);

            FunctionId func = base_func_id_
            ...
            batch.emplace_back(inv_id,
                               func,
                               tenant,
                               /*user*/ tenant,
                               /*arrival_time*/ next_arrival_time_,
                               svc);

            ++generated_;
            if (generated_ >= total_invocations_) break;
        }
...
private:
    double current_rate(TimePoint t) const noexcept {
        ...
        if (phase < 0.0) phase += burst_period_;

        double burst_duration = burst_duty_cycle_ * burst_period_;
        if (burst_duration < 0.0) burst_duration = 0.0;
        if (burst_duration > burst_period_) burst_duration = burst_period_;
