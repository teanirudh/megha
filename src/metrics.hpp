...
class MetricsCollector
{
  public:
    ...
    void on_invocation_execute(const Invocation &inv, WorkerId wid,
                               TimePoint now)
    {
        (void)wid;
    ...
    {
        std::uint64_t s = 0;
        for (auto &kv : tenant_drops_)
            s += kv.second;
        return s;
    }

    std::uint64_t arrivals_for_tenant_or_zero(TenantId t) const
    {
        return arrivals_for_tenant(t);
    }
    std::uint64_t drops_for_tenant_or_zero(TenantId t) const
    {
  ...
  private:
    ...
    std::unordered_map<std::pair<TenantId, TenantId>, double, TenantPairHash>
        first_coloc_time_;
