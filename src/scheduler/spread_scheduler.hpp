...
{
  public:
    SpreadScheduler() : rng_(std::random_device{}()) {}

    explicit SpreadScheduler(std::uint64_t seed) : rng_(seed) {}
                                ...
                                const PlatformState &state) override
    {
        ...
        for (WorkerId wid : state.workers())
        {
            ...
            bool has_same =
                std::find(wv.tenants_present.begin(), wv.tenants_present.end(),
                          inv.tenant_id()) != wv.tenants_present.end();

            candidates.push_back(Candidate{.id = wid,
