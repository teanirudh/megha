 ...
 *
 * Each worker maintains EMA-smoothed C_thrash from per-cycle deltas (LLC
 * misses and synthetic cycles) aggregated over busy containers using their
 * MicroarchProfile.
 *
 * Invocations are thrash-hot if intrinsic LLC miss rate (load+store per
 * cycle) exceeds kThrashHotThreshold. Workers that ever receive a thrash-hot
 * placement stay tainted: hot picks among tainted workers maximize C_thrash,
 * else seed taint on argmin C_thrash; cool picks are uniform random over
 * non-tainted workers, or over all candidates if every worker is tainted.
 */
class GuardianScheduler : public Scheduler
{
  public:
                                ...
                                const PlatformState &state) override
    {
        ...
        const bool cand_hot =
            m.llc_load_misses_per_cycle + m.llc_store_misses_per_cycle >
            kThrashHotThreshold;
  ...
  private:
    WorkerId pick_hot(const std::vector<WorkerId> &candidates)
    {
        ...
        if (!hot.empty())
            return pick_random(hot, true, [&](WorkerId w)
                               { return worker_state_[w].c_thrash; });

        return pick_random(candidates, false, [&](WorkerId w)
                           { return worker_state_[w].c_thrash; });
    }
    ...
    WorkerId pick_random(const std::vector<WorkerId> &pool, bool maximize,
                         Score score)
    {
        ...
        for (WorkerId wid : pool)
        {
            ...
            const bool better = maximize ? (s > best) : (s < best);
            if (better)
            {
                best = s;
                tied.clear();
    ...
    static constexpr double kThrashHotThreshold = 0.01;

    void advance_counters(WorkerId wid, const PlatformState &state,
                          TimePoint now)
    {
        WorkerState &ws = worker_state_[wid];
        ...
        for (const auto &c : w.containers)
        {
            ...
            ws.cumulative.llc_load_misses += dcyc * m.llc_load_misses_per_cycle;
